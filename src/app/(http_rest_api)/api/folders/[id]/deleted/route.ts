...
import { GetDeletedFoldersByParentController } from "./_GetDeletedFoldersByParentController";

export const GET = exceptionHandler(initzializeDependencies(async (request, route) => {
    return new AuthProxyController(new GetDeletedFoldersByParentController()).run(request, route.params);
}));
